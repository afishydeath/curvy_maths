...
import sys,math
#temp points
startPoints=[[0, 0], [50,150],[250,150],[300,0]]
###
###
...
def saveFrame(frame,fname='Frame'):
	...
	with open(f'bezierFrames\\{fname}{frame}.svg','wt') as f:
...
def run(frames,name='frame',chaos=False):
	setup(startPoints)
	hueChange=360/len(startPoints)
	for f in range(frames+1):
		progress=(f/frames)
		update_progress(progress)
		t=f/frames
		if not chaos:
			tree=frame(t,startPoints,hueChange)
			new=tree['1'][0]
			if f>0:
				maintrail(new,old)
			old=new
		else:
			chaosnew=frame(t,startPoints,hueChange)
			if f>0:
